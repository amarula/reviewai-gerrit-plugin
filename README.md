...
- `mockAiModelConfigPath`: JSON file used by the mocked model. The value can be an absolute filesystem path or a
  classpath resource. The default value is `config/mockAiModel.json`.
...
[plugin "reviewai-gerrit-plugin"]
    ...
    mockAiModel = debug
    mockAiModelConfigPath = /path/to/mockAiModel.json
```
