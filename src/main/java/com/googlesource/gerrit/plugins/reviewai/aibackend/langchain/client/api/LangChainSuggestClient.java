...
public class LangChainSuggestClient {
  ...
  public AiResponseContent ask(ChangeSetData changeSetData, GerritChange change, String patchSet)
      throws Exception {
    List<AiReplyItem> negativeReplies = new ArrayList<>();
    for (ReviewAssistantStage assistantStage : getSuggestAssistantStages(changeSetData)) {
      negativeReplies.addAll(askReviewStage(changeSetData, change, patchSet, assistantStage));
    }
    List<AiReplyItem> suggestions =
        negativeReplies.isEmpty()
            ? List.of()
            : askSuggestions(changeSetData, change, patchSet, negativeReplies);
  ...
  private List<AiReplyItem> askReviewStage(
    ...
    if (reviewResult == null || reviewResult.getResponseContent() == null) {
      return List.of();
    }

    List<AiReplyItem> negativeReplies = negativeReplies(reviewResult.getResponseContent());
    for (AiReplyItem reviewReply : negativeReplies) {
      prepareReviewLocation(reviewReply, assistantStage, patchSet);
    }
