...
class PatchSetReviewConversationRecorder {
  ...
  void record(GerritChange change, List<ReviewBatch> reviewBatches, Integer reviewScore) {
    ...
    Long timestampMillis = getEventTimestampMillis(change);
    conversationStore.appendTurn(
        change.getFullChangeId(),
        ReviewAgentConversationStore.reviewAiCommentsConversationId(changeNumber.get()),
        "ReviewAI comments",
  ...
  private Long getEventTimestampMillis(GerritChange change) {
    long eventTimestamp = change.getEventTimeStamp();
    return eventTimestamp > 0 ? eventTimestamp * 1000 : System.currentTimeMillis();
  }
}
