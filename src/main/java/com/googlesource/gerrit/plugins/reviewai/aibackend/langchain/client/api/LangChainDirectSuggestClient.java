...
class LangChainDirectSuggestClient {
  ...
  private List<AiReplyItem> prepareSuggestions(AiResponseContent responseContent, String patchSet) {
    ...
    for (AiReplyItem suggestion : responseReplies(responseContent)) {
      boolean commitMessageSuggestion = COMMIT_MESSAGE_FILENAME.equals(suggestion.getFilename());
      ...
      if (commitMessageSuggestion) {
        ...
        suggestion.setCodeSnippet(extractCommitMessage(patchSet));
      }
      if (hasSuggestionFence(suggestion)
          && (commitMessageSuggestion || hasCodeSuggestionTarget(suggestion))) {
