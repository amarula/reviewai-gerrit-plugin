...
public abstract class ClientCommandBase extends ClientBase {
  ...
  private static final ImmutableBiMap<CommandSet, String> COMMAND_MAP_INVERSE =
      COMMAND_MAP.inverse();
  public static final Set<CommandSet> GERRIT_MESSAGE_HISTORY_SKIPPED_COMMANDS =
  ...
  private static final Pattern GERRIT_MESSAGE_HISTORY_SKIPPED_COMMAND_PATTERN =
      Pattern.compile(
          "\\s*/(?:"
              + GERRIT_MESSAGE_HISTORY_SKIPPED_COMMANDS.stream()
  ...
  public static boolean shouldSkipGerritMessageHistory(String message) {
    return message != null
        && GERRIT_MESSAGE_HISTORY_SKIPPED_COMMAND_PATTERN.matcher(message).matches();
  }
}
