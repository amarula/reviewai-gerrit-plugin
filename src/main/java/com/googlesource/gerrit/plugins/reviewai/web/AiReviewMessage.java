...
public class AiReviewMessage implements RestModifyView<ChangeResource, AiReviewMessage.Input> {
  ...
  public Response<Output> apply(ChangeResource resource, Input input) throws Exception {
    ...
    Optional<Output> directResponse = getDirectResponse(resource, config, message);
  ...
  private Optional<Output> getDirectResponse(
      ChangeResource resource, Configuration config, String message) {
    if (!ClientCommandBase.shouldSkipGerritMessageHistory(message)) {
    ...
    return Optional.of(new Output(true, changeSetData.getReviewSystemMessage()));
  }
