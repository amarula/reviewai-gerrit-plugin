...
public class GerritAiReviewHistoryCollector {
  ...
  public AiReviewHistoryInfo collect(
    ...
    for (GerritComment comment : comments) {
      if (!shouldDisplay(comment, config, localizer, aiAccountId, botMentionPattern)) {
        continue;
      }
  ...
  private boolean shouldDisplay(
    ...
    if (isHiddenUserCommand(comment, config, localizer, botMentionPattern)) {
      return false;
    }
    return isDirectedToAi(comment, aiAccountId, botMentionPattern);
  }

  private boolean isHiddenUserCommand(
      GerritComment comment, Configuration config, Localizer localizer, Pattern botMentionPattern) {
    if (comment == null
        || comment.getAuthor() == null
        || comment.isAutogenerated()
        || !botMentionPattern.matcher(Optional.ofNullable(comment.getMessage()).orElse("")).find()) {
      return false;
    }
    String cleanedMessage =
        new ClientMessageCleaner(config, Optional.ofNullable(comment.getMessage()).orElse(""), localizer)
            ...
            .getMessage()
            .trim();
    return ClientCommandBase.shouldSkipGerritMessageHistory(cleanedMessage);
  }
