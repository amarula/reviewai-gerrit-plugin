...
public class GerritAiReviewHistoryCollector {
  ...
  public AiReviewHistoryInfo collect(
    ...
    entries.sort(
        Comparator.comparing(
            ...
            .thenComparing(
                AiReviewHistoryInfo.Entry::getId, Comparator.nullsLast(String::compareTo)));
  ...
  private Map<String, List<GerritComment>> toCommentsByFile(GerritClientData gerritClientData) {
    ...
    if (commentData.getPatchSetCommentMap() != null) {
      addCommentsByFile(commentsByFile, seenComments, commentData.getPatchSetCommentMap().values());
    }
  ...
  private void addConversationEntry(
    ...
    ClientMessageCleaner cleaner =
        new ClientMessageCleaner(
            config, Optional.ofNullable(comment.getMessage()).orElse(""), localizer);
  ...
  private boolean isDirectedToAi(
  ...
  private boolean shouldDisplay(GerritComment comment, int aiAccountId, Pattern botMentionPattern) {
    if (comment == null) {
      return false;
    }
  ...
  private String getAuthorName(GerritComment comment) {
    ...
    return Stream.of(
            author.getDisplayName(), author.getName(), author.getUsername(), author.getEmail())
        .filter(value -> value != null && !value.isBlank())
        .findFirst()
        .orElse("Unknown");
  }
