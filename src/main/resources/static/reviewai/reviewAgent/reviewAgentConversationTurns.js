(function(global) {
  ...
  class ReviewAgentConversationTurns {
    ...
    mergeStoredTurnsWithHistory(storedTurns, historyTurns, includeNewTurns) {
    ...
    entriesToConversationTurns(entries) {
      ...
      agentUtils.orderAssistantEntriesWithinTurns(entries).forEach(entry => {
        ...
        if (
          !currentTurn ||
          !this.turnUserQuestion(currentTurn) ||
          (this.turnUserQuestion(currentTurn) &&
    ...
    _findStoredTurnIndexForHistory(storedTurns, historyTurn, matchedStoredTurnIndexes) {
      const historyQuestion = this.turnUserQuestion(historyTurn);
      ...
      return storedTurns.findIndex((storedTurn, index) => {
        ...
        if (this.turnUserQuestion(storedTurn) !== historyQuestion) {
          return false;
        }
    ...
    _hasEquivalentStoredAssistantResponse(storedTurns, historyTurn) {
      if (!this._isAutomaticReviewQuestion(this.turnUserQuestion(historyTurn))) {
        return false;
      }
    ...
    _appendEntrySeparators(turns) {
      turns.forEach(turn => {
        ...
        responseParts.slice(0, -1).forEach(part => {
          ...
          part.text = `${part.text}${agentUtils.markdownEntrySeparator}`;
        });
      });
    }
  }
