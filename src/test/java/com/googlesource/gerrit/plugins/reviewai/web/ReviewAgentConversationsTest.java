...
public class ReviewAgentConversationsTest extends TestBase {
  ...
  private boolean hasTable(java.sql.Connection c, String tableName) throws Exception {
    try (var rs = c.getMetaData().getTables(null, null, tableName, null)) {
      return rs.next();
    }
  }

  private boolean hasColumn(java.sql.Connection c, String tableName, String columnName)
      throws Exception {
    try (var rs = c.getMetaData().getColumns(null, null, tableName, columnName)) {
      return rs.next();
    }
  }
