...
if [[ -z "${plugin_version}" || -z "${git_describe}" || "${git_describe}" == "unknown" ]]; then
  plugin_version_info="${plugin_version}"
elif [[ "${git_describe}" =~ ^(.+)(-[0-9]+-g[0-9a-f]+(-dirty)?)$ ]]; then
  plugin_version_info="${plugin_version} (build ${BASH_REMATCH[2]#-})"
elif [[ "${git_describe}" =~ ^([0-9a-f]+)(-dirty)?$ ]]; then
  plugin_version_info="${plugin_version} (build g${BASH_REMATCH[1]}${BASH_REMATCH[2]:-})"
else
  plugin_version_info="${plugin_version} (build ${git_describe})"
fi
