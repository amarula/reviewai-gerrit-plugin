...
```mermaid
flowchart TD
    A[Full current patch] --> B[PATCHSET review agent<br/>code and commit message]
    B --> C[Review replies]
...
flowchart TD
    ...
    E --> G[Historical repetition agent &#40*&#41]
...
```mermaid
flowchart TD
    A[Full current patch] --> C[PATCHSET Concern Reviewer agent]
    B[Incremental patch and stored<br/>SINGLE_AGENT / PATCHSET concerns] --> C
    C --> D[PATCHSET New Issue Finder agent]
    D --> F[Review replies]
...
| --- | --- |
| Concern data and status | `ReviewConcern`, `ConcernStatus` |
| Per-reviewer grouping | `ConcernReviewerId`, `ReviewerConcerns` |
| Per-Change ledger | `ReviewConcernLedger` |
| Shared stage payload | `ConcernWorkflowInput` |
| Status-update validation | `ReviewConcernStatusUpdater` |
| Shared Concern Reviewer and New Issue Finder orchestration | `LangChainClient` |
| Scoped-agent orchestration | `LangChainMultiAgentReviewClient` |
| Specialized-agent orchestration and collection | `LangChainSpecializedAgentReviewClient` |
| H2 persistence | `ReviewConcernStore`, `ReviewAiDb` |
| Load, clear, and post-publication save | `ReviewConcernPublisher`, `PatchSetReviewer` |
| Concern Reviewer prompts | `config/agents/common/concern-review/` |
| New Issue Finder prompts | `config/agents/common/new-issue-finder/` |
