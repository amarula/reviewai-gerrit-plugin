...
> does today, or point the adapter at a remote engine. §14.1 records the resulting arrangement and the
> licence constraint that dictates where the contract lives.
...
The contract lives in its own dependency-free artifact. It contains no Gerrit, no Spring, and — for a
non-obvious reason — no Jackson and no Gson annotations (§14.5).
...
### 14.1 Two repositories, and the licence constraint that decides where the contract lives

The engine service is developed in a **separate, proprietary repository**. This repository stays
Apache-2.0 and keeps its in-process engine, so a site can run either way.

```
this repository (Apache-2.0)              reviewai-backend (proprietary)
  reviewai-contract/     ◄─────────────────  depends on a pinned published version
  src/main/java/...          aibackend/        reviewai-engine/   (Spring Boot)
    (adapter + in-process engine)              adapter-side client, job API server
  reviewai-conformance/  ◄─────────────────  run as part of its build
...
/                          pom.xml  ← UNCHANGED: still the plugin, still not an aggregator
  ...
  reviewai-contract/       ← standalone Maven project; ZERO dependencies; Apache-2.0
  reviewai-conformance/    ← standalone Maven project; the contract's conformance suite
```
...
**The contract must live here, under Apache-2.0, and be consumed as a published versioned artifact.**

This is a licence constraint, not a preference. The contract and the tool-RPC protocol are on the
compile path of *both* sides. If they lived only in the proprietary repository, this Apache-2.0 plugin
would require a closed artifact to build and release — the open project would no longer be buildable
from public sources. The dependency direction is asymmetric and only one way is safe:

- **Proprietary → Apache-2.0 is fine.** Apache-2.0 is permissive with no copyleft, so the closed
  backend may depend on the published contract freely, including in a distributed product.
- **Apache-2.0 → proprietary is not.** It makes the open build depend on something closed.

Consequences to accept:

- A contract change requires coordinated releases: publish the contract, then bump the pinned version
  in the closed repository. This is the cross-repository friction a same-repo layout would have
  avoided; the licence boundary makes it unavoidable, and pinning keeps it visible and testable rather
  than inherited.
- CI in each repository is independent. This repository's CI must never require `reviewai-backend`.
...
### 14.2 Contract conformance — the mechanism that keeps two engines honest

Because the in-process engine (here) and the remote engine (proprietary) both implement the same
contract, nothing in either build would otherwise notice if they diverged. `reviewai-conformance`
closes that gap: an abstract test suite, published from this repository, that any `ReviewEngine`
implementation runs against its own build.

It covers what the contract promises rather than how it is built:

- every request field round-trips through the wire format without loss, including the nullable and
  polymorphic cases (`StateBootstrap`, the `past_comment_id` correlation value — §5.1);
...
- `CONTEXT NOT PROVIDED` is returned — not an exception thrown — when code context is unavailable (§12.1);
- contract-version rejection is explicit rather than silent (§16).

The in-process engine runs it here; the proprietary engine runs the same published suite. A divergence
then fails a build instead of surfacing as two subtly different reviewers.
...
A contract artifact is **not** in the shade `artifactSet` allowlist, so its classes would keep literal
`com.fasterxml.jackson.*` references while the plugin's own classes reference the relocated package.
At runtime inside Gerrit, the unrelocated references resolve against **Gerrit's own Jackson** — a
silent version-skew bug with no compile error.

Hence the hard rule: **zero Jackson *and* zero Gson annotations or types in the contract.** Field names
are the wire names; the engine configures `SNAKE_CASE`, the adapter configures
`LOWER_CASE_WITH_UNDERSCORES`. No hand-written mapping layer, and no annotation in the contract.

A consequence to budget for: the contract records are **not** byte-compatible with today's
`concern_json`, which is acceptable because the engine gets fresh tables — the plugin's existing rows
are read once, during bootstrap. Four or more existing test classes use Lombok setters on the concern
models and will need mechanical rewriting. Wide but shallow.
...
**Step 4 — Add the remote transport; publish the contract.** The adapter gains an engine-transport
selection (§4.1): in-process remains the default, and remote is opt-in per site. Concurrently, the
proprietary repository stands up the Spring Boot service against the published contract, and the
in-process engine is brought under the same conformance suite (§14.2) so both implementations are held
...
the remote engine down; (d) the bootstrap drill from §13; (e) the conformance suite passes against
**both** engines.
...
split. Step 2 must precede Step 4. Steps 1 and 2 are independent of each other. Steps 4 and 5 cross a
repository boundary, so the contract must be published (with a pinned version) before Step 5 begins.
