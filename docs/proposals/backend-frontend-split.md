...
non-obvious reason — no Jackson and no Gson annotations (§14.4).
...
public record ResolvedConfig(
    ...
    String credentialRef,            // OPAQUE. Never a token — see §14.5.
...
engine reuses the *design*, not the tables (§14.6):
...
| State | Owner | Authoritative | Who may delete |
|---|---|---|---|
...
### 14.1 Same repository, separate Maven projects

The contract lives in its own project, as do the engine and the existing plugin:

```
/                          pom.xml  ← UNCHANGED: still the plugin, still not an aggregator
  src/main/java/...        ← unchanged (57 Gerrit-coupled files stay)
  MODULE.bazel, BUILD, tools/bzl/    ← unchanged
  reviewai-contract/       ← standalone Maven project; ZERO dependencies
  reviewai-engine/         ← standalone Maven project; Spring Boot
```
...
Same repository rather than separate ones because a contract change requires coordinated test updates
on both sides; a two-repo split would need a cross-repo CI trigger for every contract change, which is
the friction that kills extractions like this.

CI must build **contract → plugin → engine** in order.

### 14.2 Bazel builds the plugin; Maven builds the engine
...
### 14.3 Keeping Gerrit off the engine classpath — enforced, not intended
...
### 14.4 The Jackson relocation trap
...
### 14.6 Reuse the design, not the tables
...
**Step 4 — Split the process.** Extract the engine to the Spring Boot module; move state to the
engine's PostgreSQL with bootstrap (§13); the adapter becomes an HTTP client.
*Proves it worked:* (a) `SIGKILL` the engine mid-review → the review retries and completes, comments
appear exactly once; (b) `SIGKILL` the adapter mid-review → the engine finishes and `EngineJobPoller`
publishes on restart, exactly once; (c) all six sidebar endpoints keep working with the engine down;
(d) the bootstrap drill from §13.

**Step 5 — Flip retention and metrics; retire the legacy tables.** Engine-side cleanup on merge/abandon;
metrics per §16. Legacy tables become read-only, then dropped once the rollback window closes.

**Ordering constraints.** Step 0 must precede Step 4 — idempotent publication is a precondition for the
split. Step 2 must precede Step 4. Steps 1 and 2 are independent of each other.
...
1. **Feedback memory's product-specific ids.** `review_feedback_comments.comment_id`
   ...
   *concern* map but not these. Either extend the adapter-owned mapping to feedback handles, or keep
   ...
   Step 4.**
2. **Where per-project LLM credentials live** post-split, given `credentialRef` (§14.5). Requires an
