...
### ReviewAI REST Endpoints

ReviewAI registers the following change-scoped REST endpoints. Replace `{change-id}` with a Gerrit change identifier
in paths of the form `/changes/{change-id}/reviewai-gerrit-plugin~<endpoint>`.

| Endpoint | Method | Build | Purpose |
| --- | --- | --- | --- |
| `ai-review-threads` | `GET` | Standard and development | Reply trees containing non-autogenerated ReviewAI comments, annotated with persisted concern-ledger and feedback-processing state. |
| `ai-review-config` | `GET` | Development only | Effective static and dynamic configuration, available models, the selected model, and review settings. |
| `ai-review-history` | `GET` | Standard and development | Filtered ReviewAI conversation history assembled from inline comments and Gerrit change messages. |
| `ai-review-agent-model` | `GET` | Standard and development | Models available to the current user, the selected default, and whether the user can invoke ReviewAI. |
| `ai-review-message` | `POST` | Standard and development | Submit a sidebar message or review request and return its request identifier or direct response. |
| `ai-review-message-status` | `POST` | Standard and development | Read the current state and result of a sidebar request by request identifier. |
| `ai-review-agent-conversations` | `POST` | Standard and development | Sidebar conversation metadata and turn operations: `list`, `get`, `upsert`, and `append`. |

`ai-review-threads` is the primary diagnostic endpoint for inspecting comment lineage and its association with the
current concern and feedback state. It returns only reply trees that contain a ReviewAI comment, and each returned
comment message is limited to 500 characters. The endpoint reads persisted state; it does not execute the review or
feedback-classification pipeline.

`ai-review-message` and the `upsert` and `append` actions of `ai-review-agent-conversations` modify state. The other
operations in the table are read-only. Endpoints that expose review data enforce the plugin's change-level ReviewAI
permissions.
